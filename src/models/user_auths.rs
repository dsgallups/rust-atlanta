...
impl Authenticable for Model {
    async fn find_by_api_key(db: &DatabaseConnection, api_key: &str) -> ModelResult<Self> {
        let auth = Entity::find()
            ...
            .one(db)
            .await?;
        auth.ok_or_else(|| ModelError::EntityNotFound)
    }
...
impl Model {
    ...
    pub async fn find_by_email(db: &DatabaseConnection, email: &str) -> ModelResult<Self> {
        let user = Entity::find()
            .filter(model::query::condition().eq(Column::Email, email).build())
            .one(db)
            .await?;
    ...
    pub async fn find_by_magic_token(db: &DatabaseConnection, token: &str) -> ModelResult<Self> {
        let user = Entity::find()
            .filter(query::condition().eq(Column::MagicLinkToken, token).build())
            .one(db)
            .await?;
    ...
    pub async fn find_by_pid(db: &DatabaseConnection, pid: &str) -> ModelResult<Self> {
        ...
        let user = Entity::find()
            .filter(model::query::condition().eq(Column::Id, parse_uuid).build())
            .one(db)
            .await?;
        user.ok_or_else(|| ModelError::EntityNotFound)
    }
