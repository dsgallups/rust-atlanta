	...
	let projects = $state<Project[]>([
		{
			...
			description: 'A comprehensive collection of command-line tools built by the Atlanta Rust community. Includes utilities for file processing, system monitoring, and development workflows.',
			author: 'sarah_dev',
		...
		},
		{
			...
			description: 'High-performance JSON/YAML parser optimized for large files. Built with zero-copy deserialization and streaming capabilities.',
		...
		},
		{
			...
			description: 'Rust wrapper for MARTA real-time transit data. Provides type-safe access to schedules, alerts, and vehicle positions.',
		...
		},
		{
			id: 4,
			name: 'Peach State DB',
			description: 'Embedded database written in pure Rust, optimized for IoT and edge computing scenarios.',
			author: 'db_wizard',
		...
		},
		{
			...
			description: 'Experimental 2D game engine built during our monthly hackathons. Features ECS architecture and WebAssembly support.',
			author: 'game_dev_atl',
		...
		},
		{
			...
			description: 'Terminal-based weather app with beautiful ASCII art visualizations for Atlanta area forecasts.',
		...
		<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			<div class="md:flex md:items-center md:justify-between">
				...
				<button
					...
					class="mt-4 md:mt-0 inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-orange-600 to-red-600 px-4 py-2 font-medium text-white transition-shadow hover:shadow-lg"
				>
			...
			<div class="flex flex-wrap gap-2">
				{#each allTags as tag}
					<button
						...
						class="rounded-full px-3 py-1 text-sm font-medium transition-colors {selectedTags.includes(tag)
		...
		{:else}
			<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				{#each filteredProjects as project}
					<div class="group relative rounded-lg border border-gray-200 bg-white p-6 transition-all hover:shadow-xl">
						{#if project.featured}
...
{#if showSubmitForm}
	<div class="fixed inset-0 z-50 overflow-y-auto">
		<div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
			...
			<!-- Modal panel -->
			<div class="inline-block w-full max-w-lg transform overflow-hidden rounded-lg bg-white text-left align-middle shadow-xl transition-all">
					...
					<form onsubmit={handleSubmitProject} class="space-y-4">
						<div>
							...
							<input
								...
								required
								class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
							/>
						</div>
						...
						<div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
							<button
								type="submit"
								class="inline-flex w-full justify-center rounded-lg bg-gradient-to-r from-orange-600 to-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-orange-500 sm:col-start-2 sm:text-sm"
							>
								Submit Project
							</button>
							<button
								type="button"
								onclick={() => showSubmitForm = false}
								class="mt-3 inline-flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-500 sm:col-start-1 sm:mt-0 sm:text-sm"
							>
								Cancel
							</button>
						</div>
